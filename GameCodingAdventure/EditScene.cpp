...
{
	if (GET_SINGLETON(InputManager)->GetButtonDown(KeyType::LeftMouse))
	{
	...
	if (GET_SINGLETON(InputManager)->GetButtonDown(KeyType::RightMouse))	
		_setOrigin = true;

	// SAVE
	if (GET_SINGLETON(InputManager)->GetButtonDown(KeyType::S))
	{
		wofstream file;
	...
	// LOAD
	if (GET_SINGLETON(InputManager)->GetButtonDown(KeyType::L))
	{
		wifstream file;
		file.open(L"Unit.txt");
